...
- **Performance — JS-layer recommendation score**: The homepage recommendation score is computed in JavaScript (not SQL) using `computeScore()` in `index.astro`. This avoids expensive per-row correlated subqueries. Badge weight comes from a LEFT JOIN on `badgeDisplays`, health score from the existing `healthMap`, freshness from `approvedAt`.
- **Performance — Batch queries**: Admin tools API (`/api/admin/tools`) uses batch `inArray` queries for tags and health checks instead of per-tool N+1 queries.
- **Performance — Cache middleware**: `src/middleware.ts` uses Cloudflare Workers Cache API (`caches.default`) to cache GET responses for `/` and `/sitemap.xml` with 60s TTL. Admin requests (with `secret` param) bypass cache. Cache writes use `ctx.waitUntil()`.
