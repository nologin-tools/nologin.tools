...
- **SEO — OG image**: Default OG image is `public/og-default.png` (1200×630, brand logo + tagline). Layout defaults to `/og-default.png`. Future: per-tool dynamic OG images.
- **SEO — Layout props**: `Layout.astro` supports optional `canonicalOverride` (string path, resolves against `https://nologin.tools`) and `publishedTime` (ISO date string for `article:published_time`). `ToolDetailPage` and `BadgeDetailPage` use `canonicalOverride` to ensure ISR pages (`/ssr/tool/*`, `/ssr/badge/*`) point canonical to `/tool/*` and `/badge/*`.
- **SEO — Sitemap lastmod**: `sitemap.xml.ts` includes `<lastmod>` for tool/badge pages (from `approvedAt`). Static pages use a fixed date.
- **SEO — JSON-LD**: Homepage has `WebSite` + `SearchAction` (supports Sitelinks Searchbox). Tool detail pages have `SoftwareApplication` (with `offers`, `operatingSystem`, `datePublished`) + `BreadcrumbList`. Badge pages have `BreadcrumbList`.
- **SEO — Meta tags**: Layout includes `og:site_name`, `og:locale`, `twitter:site` (`@nologin_tools`), explicit `twitter:title`/`twitter:description`, `theme-color` (`#22c55e`), and `<link rel="sitemap">`.
