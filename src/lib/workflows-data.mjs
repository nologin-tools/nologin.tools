...
export const WORKFLOW_RECIPES = [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        stepNumber: 3,
        toolSlug: 'hat-sh',
        actionTitle: 'Apply Client-Side AES-256-GCM Encryption',
        actionDescription: 'Upload your sanitized document to hat.sh. Enter a strong passphrase or generate an Argon2id key. The browser WebCrypto API encrypts the file into a `.enc` container locally on your CPU before any network packet can exist.',
        inputFormat: 'Sanitized PDF Document',
        outputFormat: 'Cryptographically Secured `.enc` Container',
        proTip: 'Send the decryption passphrase over a separate secure channel (e.g. Signal) to adhere to two-factor out-of-band security practices.',
      },
      {
        stepNumber: 4,
        toolSlug: 'wormhole-app',
        actionTitle: 'Distribute via Self-Destructing Link',
        actionDescription: 'Upload the encrypted `.enc` container to Wormhole. Set the link lifetime to 24 hours or 1 single download. Once fetched, the file vanishes from the intermediary transfer cache forever.',
        inputFormat: 'Encrypted `.enc` File',
        outputFormat: 'One-Time Self-Destruct Download URL',
        proTip: 'Even if the Wormhole link were intercepted in transit, the payload cannot be decrypted without the recipient holding the hat.sh password.',
      },
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        actionTitle: 'Fine-Tune Precision Vector Paths & Styling',
        actionDescription: 'Import the raw SVG into SVG-Edit. Adjust bezier control points, tweak stroke weights, align elements to a 4px layout grid, and customize branding colors directly inside the browser canvas.',
        inputFormat: 'Raw Diagram SVG',
        outputFormat: 'Polished Multi-Node SVG Illustration',
        proTip: 'Group related UI elements into distinct `<g id="...">` tags in SVG-Edit to make CSS and JavaScript styling straightforward in your web codebase.',
      },
      {
        stepNumber: 3,
  ...
  },
  {
    ...
    steps: [
      ...
      {
        stepNumber: 2,
        toolSlug: 'quick-web-tools-surge-sh',
        actionTitle: 'Validate Syntax, Format & Minify Payload',
        actionDescription: 'Load the JSON into JSON Formatter & Validator. Ensure proper RFC 8259 syntax compliance, identify unclosed braces or trailing commas, and generate beautified 2-space indented code for documentation or minified strings for API mock testing.',
