...
- **GEO — llms.txt & llms-full.txt**: `public/llms.txt` and `public/llms-full.txt` (following llmstxt.org specification) index directory purpose, core principles, 11 categories, verified tools, recommendation formula, and APIs for LLM ingestion. `sites/org/public/llms.txt` outlines organization authority, whitepapers, and research. Tests: `node --test scripts/__tests__/geo-llms.test.mjs` (6 cases).
- **GEO — AI Search Bot Directives**: `public/robots.txt` and `sites/org/public/robots.txt` explicitly configure access for generative AI crawlers (`PerplexityBot`, `OAI-SearchBot`, `ChatGPT-User`, `GPTBot`, `ClaudeBot`, `anthropic-ai`, `Google-Extended`, `Applebot-Extended`, `Meta-ExternalAgent`, `cohere-ai`), while protecting `/admin`, `/api/`, `/ssr/`, and declaring `llms-txt` alongside `sitemap.xml`. Tests: `node --test scripts/__tests__/seo-robots.test.mjs` (5 cases).
- **GEO & SEO — Tool Factsheet & Visible FAQ**: `ToolDetailPage.astro` renders an in-page Verification Details factsheet for rapid AI entity extraction and a visible FAQ section matching the `FAQPage` JSON-LD schema (preventing Google schema penalties and providing direct answer citations). Tests: `node --test scripts/__tests__/seo-geo-schema.test.mjs` (5 cases).
- **SEO — Homepage JSON-LD**: `WebSite` has `publisher` linking to `#organization`, `description`, `about` entity, and `Organization` includes `knowsAbout` topics.
- **SEO — SoftwareApplication JSON-LD**: Includes `isAccessibleForFree: true`, dynamic `applicationCategory` mapping (`AIApplication`, `DesignApplication`, etc.), `featureList`, `operatingSystem: 'All, Web Browser'`, `publisher: NoLoginTools.org`, `image` (OG image URL), `sameAs` (social/repo links), `keywords` (tag values), `offers.url` + `offers.availability`.
...
- **SEO — robots.txt**: `public/robots.txt` blocks `/admin`, `/api/`, `/ssr/` from crawlers, welcomes AI search bots, and declares sitemap and llms-txt locations.
...
Automated daily programmatic AI blog publishing has been decommissioned to prevent content saturation and ensure compliance with Google's helpful content guidelines. Blog posts are now created manually with original research, screenshots, and genuine evaluation. PRs continue to be validated by `blog-quality-check.yml` (`scripts/validate-blog-post.mjs`).
